...
const Update = () => {
    ...
    getdatarecipes()
      ...
      const getdatarecipes=async()=>{
        ...
        try{
          ...
          setpreview(rec.data.recipes[0].image)
          setrecipename(rec.data.recipes[0].recipename)
          setdesc(rec.data.recipes[0].descr)
          setimg(rec.data.recipes[0].image)
          setloading(false)
        }
        catch{
          setloading(false)
  ...
  const uploadimage=async()=>{
    try{
      if(!file1)
      {
        axios.put(`http://localhost:5000/uet/recipe/${id.id}`,{
          img:img,
          recipe:recipename,
          desc:desc,
          token:user.token   
    ...
    <div style={{width:"100%",overflow:"hidden"}}>
      ...
      <div className='mnadrecp'>
       {preview && <img src={preview}></img>}
        ...
        <input value={recipename} onChange={e=>setrecipename(e.target.value)} type="text" placeholder='Recipe name' required></input>
        <textarea value={desc} onChange={e=>setdesc(e.target.value)} placeholder='Description'></textarea>
        <button onClick={addrecipefunction}>Update</button>
      </div>
    </div>
    }
    </>
